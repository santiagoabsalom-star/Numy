 ...
 * {@link UserDetails} objects. The class is
 ...
 * @author Ben Alex
 */
public abstract class AbstractUserDetailsAuthenticationProviderWithId
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        UserDetailsWithId user = this.userCache.getUserFromCache(username);
        if (user == null) {
            cacheWasUsed = false;
            try {
    ...
    protected UserDetailsCheckerWithId getPreAuthenticationChecks() {
    ...
    protected UserDetailsCheckerWithId getPostAuthenticationChecks() {
        return this.postAuthenticationChecks;
    }

    public void setPostAuthenticationChecks(UserDetailsCheckerWithId postAuthenticationChecks) {
    ...
    private class DefaultPreAuthenticationChecks implements UserDetailsCheckerWithId {

        @Override
        public void check(UserDetailsWithId user) {
            if (!user.isAccountNonLocked()) {
                AbstractUserDetailsAuthenticationProviderWithId.this.logger
                        .debug("Failed to authenticate since user account is locked");
                throw new LockedException(AbstractUserDetailsAuthenticationProviderWithId.this.messages
                        .getMessage("AbstractUserDetailsAuthenticationProvider.locked", "User account is locked"));
            }
            if (!user.isEnabled()) {
                AbstractUserDetailsAuthenticationProviderWithId.this.logger
                        .debug("Failed to authenticate since user account is disabled");
                throw new DisabledException(AbstractUserDetailsAuthenticationProviderWithId.this.messages
                        .getMessage("AbstractUserDetailsAuthenticationProvider.disabled", "User is disabled"));
            }
            if (!user.isAccountNonExpired()) {
                AbstractUserDetailsAuthenticationProviderWithId.this.logger
                        .debug("Failed to authenticate since user account has expired");
                throw new AccountExpiredException(AbstractUserDetailsAuthenticationProviderWithId.this.messages
    ...
    private class DefaultPostAuthenticationChecks implements UserDetailsCheckerWithId {
        ...
        public void check(UserDetailsWithId user) {
            if (!user.isCredentialsNonExpired()) {
                ...
                throw new CredentialsExpiredException(AbstractUserDetailsAuthenticationProviderWithId.this.messages
